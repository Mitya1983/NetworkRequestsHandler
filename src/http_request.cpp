...
tristan::network::HttpRequest::HttpRequest(Url&& url) :
    ...
    if (!m_url.isValid() || (m_url.scheme() != "http" && m_url.scheme() != "https")){
        NetworkRequest::ProtectedMembers::pSetError(*this, tristan::network::makeError(tristan::network::ErrorCode::INVALID_URL));
        return;
    }
    m_headers.emplace(tristan::network::http::header_names::host, m_url.host());
    if (m_url.port().empty()){
...
auto tristan::network::GetRequest::requestData() -> const std::vector<uint8_t>&{
    ...
    if (!m_params.empty()){
        ...
        for (const auto& param: m_params){
            ...
            m_request_data.insert(m_request_data.end(), param.first.begin(), param.first.end());
            if (!param.second.empty()){
                m_request_data.push_back('=');
                m_request_data.insert(m_request_data.end(), param.second.begin(), param.second.end());
            }
            ++param_count;
        }
    }
    ...
    if (!m_headers.empty()){
        for (const auto& header: m_headers){
            m_request_data.insert(m_request_data.end(), header.first.begin(), header.first.end());
            m_request_data.push_back(':');
            m_request_data.insert(m_request_data.end(), header.second.begin(), header.second.end());
...
auto tristan::network::PostRequest::requestData() -> const std::vector<uint8_t>&{
    ...
    if (!m_params.empty()){
        ...
        for (const auto& param: m_params){
            ...
            body += param.first;
            body += '=';
            auto content_type = m_headers.find(tristan::network::http::header_names::content_type);
            if (content_type != m_headers.end() && content_type->second == "application/x-www-form-urlencoded"){
                body += tristan::network::utility::encodeUrl(param.second);
            }
            else{
                body += param.second;
            }
    ...
    m_request_data.insert(m_request_data.end(), tristan::network::http::header_names::content_length.begin(), tristan::network::http::header_names::content_length.end());
    m_request_data.push_back(':');
    to_insert = std::to_string(body.length());
    m_request_data.insert(m_request_data.end(), to_insert.begin(), to_insert.end());
    m_request_data.push_back('\r');
    m_request_data.push_back('\n');
    m_request_data.push_back('\r');
