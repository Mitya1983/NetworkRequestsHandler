...
tristan::network::HttpHeaders::HttpHeaders(const std::string& headers_data) {
    ...
    while (true) {
        ...
        auto header_name_end = line.find(':', new_line_start);
        if (header_name_end == std::string::npos) {
            netError("HTTP header format not conform: " + line);
            continue;
        }
        auto header_name = line.substr(new_line_start, header_name_end - new_line_start);
...
auto tristan::network::HttpHeaders::headerValue(const std::string& header_name) const
    -> std::optional< std::string > {
    auto found = std::find_if(
        m_headers.begin(), m_headers.end(), [header_name](const tristan::network::Header& header) -> bool {
...
void tristan::network::HttpHeaders::addHeader(tristan::network::Header&& header) {
    m_headers.emplace_back(std::move(header));
}

auto tristan::network::HttpHeaders::empty() -> bool { return m_headers.empty(); }

constexpr auto tristan::network::HttpHeaders::begin() noexcept
    -> std::vector< tristan::network::Header >::iterator {
    return m_headers.begin();
}

constexpr auto tristan::network::HttpHeaders::constBegin() const noexcept
    -> std::vector< tristan::network::Header >::const_iterator {
    return m_headers.cbegin();
}

constexpr auto tristan::network::HttpHeaders::end() noexcept
    -> std::vector< tristan::network::Header >::iterator {
    return m_headers.end();
}

constexpr auto tristan::network::HttpHeaders::constEnd() const noexcept
    -> std::vector< tristan::network::Header >::const_iterator {
